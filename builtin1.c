#include "shell.h"

/**
 * _customhistory - displays the history list

 * @info: Structure containing arguments.

 *  Return: Always 0

 */

int _customhistory(info_t *info)

{

    print_list(info->history);

    return (0);

}
/**
 * unset_alias - unsets alias to string

 * @info: parameter struct

 * @al: the string alias

 * Return: Always 0 on success, 1 on error

 */

int unset_alias(info_t *info, char *al)

{


    char *p, c;

    int ret;

    p = _strchr(al, '=');

    if (!p)


        return (1);

    c = *p;

    *p = 0;

    ret = delete_node_at_index(&(info->alias),

            get_node_index(info->alias, node_starts_with(info->alias, al, -1)));

    *p = c;

    return (ret);

}

/**
 * set_alias - sets alias to string

 * @info: parameter struct

 * @al: the string alias

 * Return: Always 0 on success, 1 on error

 */

int set_alias(info_t *info, char *al)

{
    char *p;

    p = _strchr(al, '=');

    if (!p)

        return (1);

    if (!*++p)

         return (unset_alias(info, al));


    unset_alias(info, al);

    return (add_node_end(&(info->alias), al, 0) == NULL);

}

/**

 * print_alias - prints an alias string

 * @alnode: the alias node

 * Return: Always 0 on success, 1 on error

 */

int print_alias(list_t *alnode)

{

    char *p = NULL, *a = NULL;

    if (node)

    {

        p = _strchr(alnode->str, '=');

        for (a = alnode->str; a <= p; a++)


        _putchar(*a);

        _putchar('\'');

        _puts(p + 1);

        _puts("'\n");

        return (0);

        }

    return (1);

}

/**
 * _customalias - mimics the alias builtin (man alias)

 * @info: Structure containing arguments. 

 *  Return: Always 0

 */

int _customalias(info_t *info)

{

        int i = 0;

        char *p = NULL;

        list_t *node = NULL;


        if (info->argc == 1)

        {

                node = info->alias;

                while (node)

                {

                        print_alias(node);

                        node = node->next;

                }

                return (0);

        }

        for (i = 1; info->argv[i]; i++)

        {

                p = _strchr(info->argv[i], '=');

                if (p)

                        set_alias(info, info->argv[i]);

                else

                        print_alias(node_starts_with(info->alias, info->argv[i], '='));

        }


        return (0);

}
